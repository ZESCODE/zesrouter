/**
 * ZESRelay - Background Service Worker
 * 
...
(function() {
    ...
    try {
      chrome.tabs.sendMessage(state.tabId, {
        ...
        if (chrome.runtime.lastError) {
          state.lastTokenError = 'sendMessage: ' + chrome.runtime.lastError.message;
          state.tabId = null;
          pushToServer();
          return;
        }
        if (resp && resp.token) {
          state.lastTokenError = '';
          addToken(resp.token, resp.action);
          pushToServer();
        } else if (resp && resp.error) {
          state.lastTokenError = 'injector: ' + resp.error;
          pushToServer();
        } else {
          state.lastTokenError = 'no response';
          pushToServer();
        }
      });
    } catch(e) {
  ...
  // ========== 向 content script 请求 V2 token（prompt failed 升级） ==========
    ...
    return new Promise(function(resolve) {
      try {
        chrome.tabs.sendMessage(state.tabId, { type: 'NEED_V2_TOKEN' }, function(resp) {
          if (chrome.runtime.lastError) {
            state.lastTokenError = 'v2 sendMessage: ' + chrome.runtime.lastError.message;
            state.tabId = null;
            resolve(false);
            return;
          }
          if (resp && resp.token) {
            state.v2Token = { token: resp.token, ts: Date.now() };
            state.lastTokenError = '';
            console.log(TAG, 'Got V2 token:', resp.token.length, 'chars');
            pushToServer();
            resolve(true);
          } else {
            state.lastTokenError = 'v2 injector: ' + (resp && resp.error ? resp.error : 'no response');
            resolve(false);
          }
        });
      } catch(e) {
  ...
  async function pushToServer() {
    ...
    try {
      ...
      var data = {
        ...
        token_error: state.lastTokenError || '',
        models: state.models,
      };
  ...
  // MV3 service worker 会在空闲 ~30s 后休眠，setInterval 停止。
  // 用 chrome.alarms 定期唤醒 SW，保证持续推送。
  function ensureAlarms() {
    chrome.alarms.create('arena-push', { periodInMinutes: 0.5 });
    chrome.alarms.create('arena-token', { periodInMinutes: 1.5 });
  }
  chrome.alarms.onAlarm.addListener(function(alarm) {
    if (alarm.name === 'arena-push') {
      pushToServer();
    } else if (alarm.name === 'arena-token') {
      cleanTokens();
      if (state.v3Tokens.length < 5) requestToken();
    }
  });
